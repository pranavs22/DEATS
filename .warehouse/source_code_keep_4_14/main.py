...
if m_deg_set != None:
    ...
    multi_degs_d, deats_fp_d, metadata_fp_d = mkdegs_dict(m_deg_set) #{cluster:(ensdarg,pete_score)}
...
### PARSE SINGLE DEGS TEXT
    ...
    # create deg list
    sing_deg_l = mkdeg_list(s_deg_set)
    # print metadata
    ...
    uncounted_deat_d = ensdarg_to_term_deg(sing_deg_l, stage_anat_d[stage_name]) #{cluster:[term,term]}
    # create an anatomy terms:count dict for each cluster
...
-single_set files/deg.tsv \
-zfa_data files/zfa_ids.txt \
-bspo_data files/bspo_cleaned.txt \
...
./main.py \
-zfin_data files/zfin_wt_expression.json \
-multiple_sets ../../../bgmp_proj/Test_Files/test.tsv \
